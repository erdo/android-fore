...
- When any data in you model changes, inside the model code call **notifyObservers()** after the state has changed.
...
Step 1) First we hook up the **add item button** so in the onclick listener we: call basket.addItem(), and then we just call an updateTotalPrice() method which updates the amount shown in the total field (no point in syncing the whole basket view here).

Step 2) Then when we hook up the **remove item button** we do something similar: call basket.removeItem(), and the call the updateTotalPrice() method (again, no point syncing the entire view here)

Step 3) The designers decide they want to display the **total number of items in the basket** as well as the price, so now we add a updateTotalNumberOfItemsInBasket() method, which does what you think it does. Of course, we need to hook that up with the Add and Remove buttons so that they now both call updateTotalPrice(); and then updateTotalNumberOfItemsInBasket();

Step 4) Now we get to the **apply discount** checkbox, if the box is checked, the discount is applied, if not there is no discount applied. Remember the model calculations have already been written and tested so what we need in the click listener is: basket.setDiscount(applyDiscount); then updateDiscount() which just shows the discount that has been applied. We also need to call updateTotalPrice() as that will have chaged, but not updateTotalNumberOfItemsInBasket() because of course discouts have no effect there.
...
Here is the psuedo code we end up with for this (very over simplified) case
    ...
    private void setupButtonListeners() {
        ...
        addItemButton.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                basket.addItem();
                updateTotalPrice();
                updateTotalNumberOfItemsInBasket();
            }
        });
    ...
    private void updateTotalNumberOfItemsInBasket(){
...
I'm guessing you have gone back and spotted the bug by now, but in case you haven't you can recreate it by selecting the discount checkbox first and then adding or removing an item. It's that simple. The add and remove item click listeners will correctly talk to the model, so the model state is correct. However the developer forgot to call updateDiscount() so this value will be incorrect in the view until the discount checkbox is toggled again.

So very simple views can easily become a complete mess and some UI consistency bugs are very difficult to spot. Luckily there is a very simple solution and all you have to do is apply it everywhere you have a view.
...
It's not good enough to just set a button as **disabled** if a total is 0 or less. You must also set that button as **enabled** if it is greater than 0. If you don't set an affirmative step for both the positive and negative scenarious, then you run the risk of a syncView() call not setting any state, which means the result will be undeterministic (it will be whatever state it had previously).
...
10) **A syncView() that is more than 5-10 lines long and/or doesn't have one line to set an affirmative each for each propery of each UI element you are concered with**. Take a look at the who to write a good syncView() method under the data binding section.
...
Try to use these observers to just notify client code of changes to the model's state (once the client code has been told there are changes, it can use fast returning getXX() methods on the model to find out what actually happened, redraw it's state or whatever - if this isn't straight forward then the models you have implemented probably need to be refactored slightly). For some, this is a strange way to develop, but once you've done it once or twice and you understand it, the resulting code is rock solid and very compact.

If you want a library that lets you send data in these observables, you should look at RxJava which gives you enough rope to hang yourself with (and then some). In some ways that library is similar to this one (both are an implementation of the Observer pattern and let you subscribe to notifications from data sources) but RxJava has a huge and flexible API, and implements many cool features (none of which we have so far ever required - even the most complicated android app is still just some logic and data with a UI put on top of it. Once you remove the crazy, turns out it's not rocket science after all). Because ASAF has been private for so long, the API has actually gotten smaller over the years rather than larger (as we realised we were trying to be too clever).
...
Alternatively the network call might be fetching some data for the model as in the following:
