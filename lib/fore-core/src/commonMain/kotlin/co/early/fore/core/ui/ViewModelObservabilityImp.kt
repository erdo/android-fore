 ...
 *
 * 1) Observable (e.g. by a fragment or activity)
...
class ViewModelObservabilityImp(
    ...
    /**
     * @param observer (uiToVmObserver) this will typically be a fragment or activity observer which is observing the view model
     */
    override fun addObserver(observer: Observer) {
        verifyInitialization()
        uiToVmObservable.addObserver(observer)
    ...
    override fun removeObserver(observer: Observer) {
        verifyInitialization()
        uiToVmObservable.removeObserver(observer)
