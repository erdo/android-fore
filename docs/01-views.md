...
The SyncTrigger ([kotlin](https://github.com/erdo/android-fore/blob/master/fore-kt-core/src/main/java/co/early/fore/kt/core/ui/SyncTrigger.kt) \| [java](https://github.com/erdo/android-fore/blob/master/fore-jv-core/src/main/java/co/early/fore/core/ui/SyncTrigger.java)) lets you create an event like an animation that must be fired only once, from inside the syncView() method (which can be called at any time, [an arbitrary number of times](https://erdo.github.io/android-fore/05-extras.html#notification-counting)).
