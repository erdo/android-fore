...
* 1) If the **model** is later refactored to contain some other piece of state that changes, it will result in an additional call to syncView() that your view was not prepared for.
* 2) If the **view** is later refactored to observe an additional model, that additional model will also result in a call to syncView() whenever that additional model changes, and you will get more syncView() calls than you were prepared for.
