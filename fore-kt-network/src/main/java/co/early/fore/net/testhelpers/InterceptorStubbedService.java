...
public class InterceptorStubbedService implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (!stubbedServiceDefinition.successfullyConnected()) { //stubbing a connection failure
            throw stubbedServiceDefinition.getIoException();
        }

        String bodyString = JunitFileIO.getRawResourceAsUTF8String(stubbedServiceDefinition.getResourceFileName(), this.getClass().getClassLoader());

        return new Response.Builder()
                    .request(chain.request())
                    .protocol(Protocol.HTTP_1_1)
                    .code(stubbedServiceDefinition.getHttpCode())
                    .addHeader("Content-Type", stubbedServiceDefinition.getMimeType())
                    .body(ResponseBody.create(MediaType.parse(stubbedServiceDefinition.getMimeType()), bodyString))
                    .message("")
                    .build();
    }
